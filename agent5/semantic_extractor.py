...
This module identifies atomic semantic actions at the deepest AST/CFG level:
- validation
- permission check
- state mutation
- irreversible side effect
...
NO MERMAID, NO HIERARCHY YET, NO LLM INFERENCE.
"""
...
from agent5.clang_ast_parser import ASTNode, FunctionInfo, NodeType, CallGraph
from agent5.cfg_builder import ControlFlowGraph, BasicBlock, EdgeType
...
class SemanticAction:
    """
    Represents an atomic semantic action extracted from the code.
    This is the fundamental unit of understanding at the leaf level.
    """
    action_type: SemanticActionType
    effect: str  # Human-readable description of what this action does
    control_impact: bool  # Does this affect control flow?
    state_impact: bool  # Does this affect persistent state?
    source_location: str  # Where in the code this action occurs
    code_snippet: str = ""  # Optional code snippet
    guard_condition: Optional[str] = None  # If this is a decision, what's the condition?
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "guardCondition": self.guard_condition,
...
class FunctionSemantics:
    """
    Semantic description of a function at the leaf level.
    Contains all atomic actions extracted from the function.
    """
    function_name: str
    is_leaf: bool
