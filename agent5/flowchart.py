...
def write_flowchart(
    ...
    *,
    ...
    This function uses libclang-generated JSON AST.
    
    Args:
        output_path: Path to output .mmd file
        file_path: Path to entry file (optional, for reference only)
        function_name: Entry function name (optional, auto-detect if not specified)
        ast_json_path: Path to JSON AST file (required, generated by build-ast command)
        project_path: Project root (optional, not used but kept for compatibility)
        max_steps: Maximum steps
        detail_level: Detail level (high|medium|deep)
        use_llm: Use LLM for translation
        chat_model: Chat model name
    ...
    Raises:
        FileNotFoundError: If AST JSON file doesn't exist
    """
    ast_json_path = Path(ast_json_path)
    if not ast_json_path.exists():
        raise FileNotFoundError(
            f"AST JSON file not found: {ast_json_path}. "
            "Please run 'python -m agent5 build-ast' first to generate the JSON file."
        )
